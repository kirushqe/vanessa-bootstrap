...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Иначе	
		...
		КонецЕсли;
	КонецЕсли;
	Форма.Элементы.ЗаполненностьДекорация.Картинка = ?(Форма.ТребуетЗаполнения, БиблиотекаКартинок.Предупреждение, БиблиотекаКартинок.Информация2);
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				...
				ИнформационнаяКартинка = БиблиотекаКартинок.Информация2;
			КонецЕсли;
		Иначе
